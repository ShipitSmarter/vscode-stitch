...
export class PdfPreview extends Disposable implements vscode.Disposable {
    ...
    private static _getHtml(webview: vscode.Webview, extensionUri: vscode.Uri): string {
        ...
        const resolveAsUri = (...p: string[]): vscode.Uri => {
            return webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, ...p));
        };
        ...
        <html dir="ltr" mozdisallowselectionprint>
          <head>
            ...
            <meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src 'self' data:; connect-src ${cspSource}; script-src 'unsafe-inline' ${cspSource}; style-src 'unsafe-inline' ${cspSource}; img-src blob: data: ${cspSource};">
            <title>PDF.js viewer</title>
        
        
            <link rel="stylesheet" href="${resolveAsUri('lib', 'pdfjs', 'web', 'viewer.css')}">
            <link rel="stylesheet" href="${resolveAsUri('assets', 'pdf.css')}">
        ...
        <script src="${resolveAsUri('lib', 'pdfjs', 'build', 'pdf.worker.js')}"></script>
          ...
          <script src="${resolveAsUri('assets', 'pdf-main.js')}"></script>
