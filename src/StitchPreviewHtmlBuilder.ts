...
import {
  ...
  HttpMulipartStepConfiguration,
  HttpStepConfiguration,
  MailStepConfiguration,
  RenderTemplateStepConfiguration,
  SftpStepConfiguration,
  StepConfiguration,
  Base64EncodeStepConfiguration,
  CacheLoadStepConfiguration,
  CacheStoreStepConfiguration,
} from "./types/stepConfiguration";
import {
  BaseStepResult,
  LoopStepResult,
  ...
  const stepsHtml = Object.keys(response.integrationContext.steps)
    .map((key) => {
      return _createStepHtml(
        ...
        response.stepConfigurations[key] ??
          <BaseStepConfiguration>{ id: key, template: "" }
      );
    })
    .join("");
...
function _createResponseHtml(
  response: EditorSimulateIntegrationResponse
): string {
  ...
  if (response.result.headers) {
    body += `<p>${Object.keys(response.result.headers)
      .map((key) => `${key}:&nbsp;${response.result.headers?.[key]}<br />`)
      .join("")}</p>`;
  }

  if (response.validFormat === false) {
    body += `<p>Format error:&nbsp;&nbsp;<span class="errormessage">${_replaceNewlines(
      response.formatErrorMessage
    )}</span></p>`;
  }
...
function _createNavHtml(steps: Record<string, StepResult>): string {
  const stepsNav = Object.keys(steps)
    .map((key) => {
  ...
  switch (step.$type) {
    case CONSTANTS.httpStepResultTypeType:
      return _createActionStepHtml(
        ...
        configuration.$type === CONSTANTS.httpMultipartStepConfigurationType
          ? _getHttpMultipartStepHtml(
              <HttpMulipartStepConfiguration>configuration
            )
          : _getHttpStepHtml(
              <HttpStepResult>step,
              <HttpStepConfiguration>configuration
            ),
        (<HttpStepConfiguration>configuration).validFormat ?? true
          ? "action"
          : "actionerror"
      );
    case CONSTANTS.renderTemplateStepResultType:
      return _createActionStepHtml(
        "RenderTemplate",
        configuration,
        _getRenderTemplateStepHtml(
          <RenderTemplateStepResult>step,
    ...
    case CONSTANTS.sftpStepResultType:
      return _createActionStepHtml(
        "SFTP",
        configuration,
        _getSftpStepHtml(<SftpStepConfiguration>configuration)
      );
    ...
    case CONSTANTS.loopStepResultType:
      return _createActionStepHtml(
        "Loop",
        configuration,
        _getLoopStepHtml(<LoopStepResult>step)
      );
    case CONSTANTS.base64EncodeStepResultType:
      return _createActionStepHtml(
        "Base64Encode",
        configuration,
        _getBase64EncodeStepHtml(<Base64EncodeStepConfiguration>configuration)
      );
    case CONSTANTS.cacheLoadStepResultType:
      return _createActionStepHtml(
        "CacheLoad",
        configuration,
        _getCacheLoadStepHtml(<CacheLoadStepConfiguration>configuration)
      );
    case CONSTANTS.cacheStoreStepResultType:
      return _createActionStepHtml(
        "CacheStore",
        configuration,
        _getCacheStoreStepHtml(<CacheStoreStepConfiguration>configuration)
      );
    default:
      return _createActionStepHtml(
        "Unknown",
        configuration,
        _getDefaultStepHtml(step, configuration)
      );
  }
}

function _createActionHtml(
  title: string,
  type: string,
  anchor: string,
...
function _createActionStepHtml(
  title: string,
  ...
  if (step.$type === CONSTANTS.httpMultipartStepConfigurationType) {
    templateCode =
      "Please use the 'Create HTTP request' button to view entire content";
  }
  ...
  return _createActionHtml(
    ...
    title,
    step.id,
    `{action: ${CommandAction.viewStepRequest}, content: '${step.id}' }`,
    actionHtmlBody,
    cssClass
  );
}

function _getDefaultStepHtml(
  step: BaseStepResult,
...
function _getHttpStepHtml(
  step: HttpStepResult,
  configuration: HttpStepConfiguration
) {
  let html = `<p>${configuration.method} ${configuration.url}</p>`;
  if (configuration.headers) {
    html += `<p>${Object.keys(configuration.headers)
      .map((key) => `${key}:&nbsp;${configuration.headers?.[key]}<br />`)
      .join("")}</p>`;
  }
  html += `<p>`;

  if (configuration.validFormat === false) {
    html += `Format error:&nbsp;&nbsp;<span class="errormessage">${_replaceNewlines(
      configuration.formatErrorMessage
    )}</span><br />`;
  }
  ...
  if (configuration.retries) {
    ...
    if (configuration.retries.delaysMs) {
      html += `&nbsp;&nbsp;&nbsp;DelaysMs:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[${configuration.retries.delaysMs.join(
        ", "
      )}] ms<br/>`;
    }
...
function _getHttpMultipartStepHtml(
  configuration: HttpMulipartStepConfiguration
) {
  let html = `<p>${configuration.method} ${configuration.url}</p>`;
  if (configuration.headers) {
    html += `<p>${Object.keys(configuration.headers)
...
function _getRenderTemplateStepHtml(
  step: RenderTemplateStepResult,
  configuration: RenderTemplateStepConfiguration
) {
  let html = "";
  if (configuration.additionalFiles) {
    html += `<dl>
                         <dt>Additional files</dt>
                         ${configuration.additionalFiles
                           .map((f) => `<dd>${f}</dd>`)
                           .join("")}
                     </dl>`;
  }
...
function _getMailStepHtml(configuration: MailStepConfiguration) {
  return `<p>
                ...
                ReplyTo:&nbsp;&nbsp;&nbsp;&nbsp;${configuration.replyToList.join(", ")}<br />
                To:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${configuration.to.join(", ")}<br />   
            </p>`;
}

function _getBase64EncodeStepHtml(
  configuration: Base64EncodeStepConfiguration
) {
  return `<p>
...
function _replaceNewlines(text: string): string {
  if (!text) {
    return text;
  }
  return text.replace(/\n/g, "<br />  - ");
